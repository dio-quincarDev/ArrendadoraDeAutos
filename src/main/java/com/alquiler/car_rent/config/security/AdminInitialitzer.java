...
public class AdminInitialitzer implements CommandLineRunner {
    ...
    public void run(String... args) throws Exception {
        if (!userEntityRepository.existsByRole(Role.ADMIN)) {
            log.debug("No ADMIN user found. Creating initial admin.");
            ...
            if (username == null || password == null || email == null) {
                log.error("Initial admin user properties (username, password, email) are not fully configured. Skipping creation.");
                return;
            }
            ...
            log.info("Initial ADMIN user '{}' created successfully.", username);
